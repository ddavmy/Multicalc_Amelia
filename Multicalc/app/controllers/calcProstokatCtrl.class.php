...
class calcProstokatCtrl {
    ...
    public function ProstokatDelete() {
    ...
    public function action_Prostokat(){
        $this->getParams();
        $submit = ParamUtils::getFromRequest('submit', true);
        App::getMessages()->clear();
        if($submit == "Oblicz") {
            $this->ProstokatCompute();
        }else if($submit == "Usuń" && $this->form->role == "admin") {
            $this->ProstokatDelete();
        }else{
            $this->WynikList();
        }
	}

    public function ProstokatCompute(){
        if ($this->validate()) {
            ...
            $this->result->wynikA = round($this->form->dlugoscA * $this->form->dlugoscB, 2);
            Utils::addWynikMessage('Pole = '.$this->result->wynikA);

            $this->result->wynikB = 2 * round($this->form->dlugoscA + $this->form->dlugoscB, 2);
    ...
    public function generateView(){
        ...
        App::getSmarty()->display('calcProstokat.tpl');
    }
