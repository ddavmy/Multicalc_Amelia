...
class calcPOrbCtrl {
    ...
    public function action_POrbDelete() {
        if ($this->validateEdit()) {

            try {
                App::getDB()->delete("calc__porb", [
                    "id" => $this->form->id
                ]);
    ...
    public function wynikList() {
        ...
        $this->log("ROLA=".$role);
        if($role == 1){
            $this->records = App::getDB()->select("calc__porb", [
    ...
    public function action_POrbShow(){
        $this->wynikList();
	}

    public function action_POrbCompute(){
        if ($this->validate()) {
            ...
            $this->obwod = round(4 * $this->form->a, 2);
    ...
    public function generateView(){
        ...
        App::getSmarty()->display('calcPOrb.tpl');
    }
