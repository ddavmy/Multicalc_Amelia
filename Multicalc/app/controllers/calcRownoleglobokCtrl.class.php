...
class calcRownoleglobokCtrl {
    ...
    public function RownoleglobokDelete() {
    ...
    public function action_Rownoleglobok(){
        $this->getParams();
        $submit = ParamUtils::getFromRequest('submit', true);
        App::getMessages()->clear();
        if($submit == "Oblicz") {
            $this->RownoleglobokCompute();
        }else if($submit == "Usuń" && $this->form->role == "admin") {
            $this->RownoleglobokDelete();
        }else{
            $this->WynikList();
        }
	}

    public function RownoleglobokCompute(){
        if ($this->validate()) {
            ...
            $this->result->wynikA = round($this->form->dlugoscA * $this->form->dlugoscC, 2);
            Utils::addWynikMessage('Pole = '.$this->result->wynikA);

            $this->result->wynikB = round(2 * ($this->form->dlugoscA + $this->form->dlugoscB), 2);
    ...
    public function generateView(){
        ...
        App::getSmarty()->assign('pole',$this->result->wynikA);
        App::getSmarty()->assign('obwod',$this->result->wynikB);
        App::getSmarty()->assign('records',$this->result->records);
        
        App::getSmarty()->display('calcRownoleglobok.tpl');
    }
